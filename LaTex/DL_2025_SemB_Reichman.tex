...
(2) ensuring fair comparison by keeping training procedures consistent across models, and 
(3) dealing with computational efficiency and resource constraints, since the ViT required significantly more time and memory to train compared to the CNN.
...
Figure~\ref{fig:cnn_training_losses} and Figure~\ref{fig:vit_training_losses} show the training loss over epochs. CNN achieved faster convergence and outperformed ViT across all metrics, including recall, while ViT required longer training but still lagged behind CNN.
...
\begin{itemize}
    \item \textbf{CNN efficiency:} The CNN baseline trained significantly faster and achieved high accuracy in fewer epochs. Its strong inductive bias for local features made it effective for detecting pneumonia patterns localized in lung regions.
    \item \textbf{ViT capacity:} The Vision Transformer required more training time and data to reach its performance, reflecting its higher model complexity, but it can capture more global context across the X-ray image.
    \item \textbf{Overfitting behavior:} The CNN generalized more stably on the small dataset, whereas the ViT showed stronger fitting capacity but tended to overfit due to its higher complexity and data requirements.
    \item \textbf{Model complexity:} CNN contains roughly 404k parameters, while ViT contains ~14.4M, explaining the longer training time and larger memory requirements.
    \item \textbf{Interpretability:} ViT attention maps provided more interpretable visualizations, highlighting relevant lung regions, while CNN feature maps were less intuitive.
\end{itemize}

Overall, CNNs remain a strong and efficient baseline for medical imaging tasks, while ViTs represent a promising direction, especially when pretrained on large datasets or when interpretability is important.
